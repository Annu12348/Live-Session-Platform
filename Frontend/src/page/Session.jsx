...
const Session = () => {
  ...
  const [loading, setLoading] = useState(true);
  const [muted, setMuted] = useState(false);
  const [cameraOff, setCameraOff] = useState(false); // student camera
  const [teacherCameraOff, setTeacherCameraOff] = useState(false); // teacher camera
  ...
  const handleJoinSession = async () => {
    ...
    try {
      ...
      socket.on("camera-toggle", ({ isCameraOn }) => {
  ...
  const toggleCamera = () => {
    ...
    socketRef.current.emit("camera-toggle", { isCameraOn: !newState });
  };
          ...
          <>
            <div className="relative grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="relative">
                {cameraOff ? (
                  <div className="bg-gray-800 w-full h-60 md:h-80 rounded-lg flex items-center justify-center text-white text-3xl">
                    You
                  </div>
                ) : (
                  <video
